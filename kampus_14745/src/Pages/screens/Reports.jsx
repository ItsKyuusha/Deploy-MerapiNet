...
const Reports = () => {
  ...
  useEffect(() => {
    const fetchReports = async () => {
      try {
        const response = await axios.get("https://api-merapinet.vercel.app/api/reports", {
  ...
  const handleAddReport = async () => {
    try {
      const response = await axios.post(
        "https://api-merapinet.vercel.app/api/reports",
        newReport,
        {
          headers: { Authorization: `Bearer ${token}` },
        }
  ...
  const handleEditReport = (report) => {
    Swal.fire({
      ...
      if (result.isConfirmed) {
        ...
        axios
          .put(`https://api-merapinet.vercel.app/api/reports/${report.id}`, updatedReport, {
            headers: { Authorization: `Bearer ${token}` },
          })
          .then(() => {
  ...
  const handleDeleteReport = (reportId) => {
    Swal.fire({
      ...
      if (result.isConfirmed) {
        axios
          .delete(`https://api-merapinet.vercel.app/api/reports/${reportId}`, {
            headers: { Authorization: `Bearer ${token}` },
          })
          .then(() => {
